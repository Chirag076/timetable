...
export default function AdminPanel() {
  ...
  // ---------- Batch Type ----------
  const [batchType, setBatchType] = useState("morning"); // morning or evening
  ...
  const addFaculty = async () => {
    try {
      await axios.post("http://localhost:5000/api/faculty", {
        ...
        // expanded availability so morning 08:10 period is allowed
        availability: [
  ...
  const generateTimetable = async () => {
    try {
      await axios.post("http://localhost:5000/api/timetable/generate", {
        batchType, // backend calculates periods dynamically
      });
  ...
  return (
    <div style={{ padding: "20px" }}>
      ...
      <div style={{ marginBottom: "20px" }}>
        ...
        <input placeholder="Subjects (comma separated)" value={facultySubjects} onChange={e => setFacultySubjects(e.target.value)} />
        <input type="number" placeholder="Max Hours per Week" value={facultyMaxHours} onChange={e => setFacultyMaxHours(e.target.value)} />
      ...
      <div style={{ marginBottom: "20px" }}>
        ...
        <input type="number" placeholder="Number of Students" value={students} onChange={e => setStudents(e.target.value)} />
      ...
      <div style={{ marginBottom: "20px" }}>
        ...
        <input type="number" placeholder="Capacity" value={roomCapacity} onChange={e => setRoomCapacity(e.target.value)} />
